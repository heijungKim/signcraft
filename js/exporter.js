...
window.Exporter = (function () {
  ...
  function filename(spec, ext) {
    return `signcraft_${PRODUCTS.label(spec.productType)}_${spec.widthMM}x${spec.heightMM}.${ext}`;
  }
  ...
  function exportSVG(spec) {
    download(filename(spec, "svg"), new Blob([buildSVG(spec)], { type: "image/svg+xml" }));
  }
